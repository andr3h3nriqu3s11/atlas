...
.handle(async (req: FastifyRequest<{Body: AddSkillCharacter}>, reply): Promise<Character> => {
    const {body} = req;
...
.handle(async (req: FastifyRequest<{Body: SkillCharacterPair}>, reply): Promise<Character> => {
    const {body} = req;
    const {campaign, token: {user}} = await req.authenticate_verifyCampaign(body.campaign_id, true);
    ...
    if (campaign.type === CampaignType.SWADE) {
        ...
        await prisma.sWADE_CharacterSheet_Skill.delete({ where: { id: skill_pair_id } });
